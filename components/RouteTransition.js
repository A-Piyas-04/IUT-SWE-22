...
export default function RouteTransition({
  ...
  const ease = [0.22, 1, 0.36, 1]; // cubic-bezier ease-out, crisp but natural
  ...
  const variants = useMemo(() => {
    ...
    switch (type) {
      case "fade-scale":
        return {
          initial: { opacity: 0, scale: 0.985, filter: "saturate(0.98)", willChange: "opacity, transform" },
          enter: { opacity: 1, scale: 1, filter: "saturate(1)", transition: { duration, ease } },
          exit: { opacity: 0, scale: 0.985, filter: "saturate(0.98)", transition: { duration, ease } },
        };
      case "fade-slide":
      default:
        return {
          initial: { opacity: 0, x: direction * 12, willChange: "opacity, transform" },
          enter: { opacity: 1, x: 0, transition: { duration, ease } },
          exit: { opacity: 0, x: direction * -12, transition: { duration, ease } },
        };
    }
  ...
  return (
    <div aria-live="polite" aria-atomic="true" role="region" aria-busy={busy} style={{ position: "relative" }}>
